...
class DetailViewController: UIViewController, UITextFieldDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {        
        if segue.identifier == "ingredient" {
            let destinationController = segue.destinationViewController as! IngredientsViewController
            destinationController.item = item!
        }
        if segue.identifier == "Instruction" {
            let desController = segue.destinationViewController as! InstructionViewController
